  ...
  <div class="relative w-full">
    ...
    <div class="absolute navigation flex w-full h-full justify-between">
      <div class="toggle-page my-auto">
        <i @click="preSlide" class="fas fa-chevron-left" v-if="navigationEnabled"></i>
      </div>
      <!-- pagination -->
      <div class="flex gap-2 p-2 mt-auto" v-if="paginationEnabled">
        <span class="cursor-pointer w-4 h-4 rounded-full bg-white shadow-sm" @click="goToSlide(index)"
          v-for="(item, index) in getSlideCount" :key="index" :class="{ 'bg-green-200': index + 1 === currentSlide }">
        </span>
      </div>
      <!-- end pagination -->
      <div class="toggle-page my-auto">
        <i @click="nextSlide" class="fas fa-chevron-right" v-if="navigationEnabled"></i>
      </div>
    </div>
    <!-- end navigation -->
  </div>
